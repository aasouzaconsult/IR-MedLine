...
def search_expanded(query, terms_dt, matrix_tt):
    terms = []
    for i in query:
        if i in terms_dt: # texto
            key = terms_dt.index(i) # Pega a posicao que o termo se encontra
            terms_recommended = np.sort(matrix_tt[key])[:, len(matrix_tt)-expansion:len(matrix_tt)] # Final da 400 os 5 ultimos (maiores)
...
def search (query, terms_dt, matrix_tt):
    ...
    for i in query:
        if i in terms_dt:
            ...
            print key
            termss_recommended = np.sort(matrix_tt[key])[:, matrix_tt[key]]
            print termss_recommended
            #for j in termss_recommended.tolist()[0]:
...
def main():
    ...
    for i in xrange(0,len(querys)):
	   ...
	   #query_token = tokenize_stopwords_stemmer('the crystalline lens in vertebrates, including humans.', stemmer)
